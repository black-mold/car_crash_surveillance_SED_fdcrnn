...
# training / validation tensorboard directory
    ...
    log_dir = os.path.join(log_dir, args.data + time.strftime('_%Y_%m_%d_%I_%M',time.localtime(time.time()))+ '_th_07' ,args.mode) # ./runs\MIVIA_2022_07_26_11_46\train
...
model = CRNN(**conf["CRNN"]).to(device)
# import pdb
# pdb.set_trace()
# print(model)
loss_fn = nn.BCELoss()
...
def train(dataloader, model, loss_fn, optimizer, epoch):
    ...
    for batch_idx, (X, y) in enumerate(dataloader, 0): 
        ...
        # X : [batchsize, frequency, frame] & [batchsize, 128, 3751]
        # y :[batchsize, frame, class]

        # 예측 오류 계산
        pred, _ = model(X) 

        # print(f"X    : {X.shape}")    # [batchsize, 128, 3751]
        # print(f"pred : {pred.shape}") # [batchsize, 937, 3]
        # print(f"y    : {y.shape}")    # [batchsize, 937, 3]
        loss = loss_fn(pred, y)  
...
def test(dataloader, model, loss_fn):
    ...
    with torch.no_grad():
        for i, (X, y) in enumerate(dataloader):
            ...
            pred, _ = model(X)                # pred : [batchsize, frame, class]
